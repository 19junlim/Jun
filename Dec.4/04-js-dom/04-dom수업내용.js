...
// const list = document.querySelectorAll("ul > li"); // 유사 배열 (NodeList) //ul에있는 모든 li
// console.log(list); // [li, li#hoon, li, li, li]

// 1) style 속성
// - DOM 요소의 인라인 스타일을 지정할 수 있도록 제공되는 속성.
// - style.XXX (XXX: camelCase)
// list[0].style.backgroundColor = "purple"; //  background-color: ; // 여러가지 중에서 한가지 또는 소수만 바꾸고 싶을때는 이방법이 더 좋디
// list[0].style.fontSize = "20px";
// list[0].style.color = "white";

// 반복문 사용해서 적용 (list 배열에 포함된 모든 요소) // 이건 여러가지를 한번에 바꾸고싶을떄 사용한다
// for (let li of list) {
...
//둘다 메모리에서 즉시 삭제되지 않으니, 참조를 유지하면 재활용이 가능하다.
